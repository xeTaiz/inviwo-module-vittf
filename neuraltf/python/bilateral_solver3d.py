...
def filter_box_separated(input, size=3):
    win = torch.ones(size)[None, None, None, None].to(input.dtype).to(input.device) / size
    p = size // 2
    out = F.conv3d(input, win, groups=input.size(1), padding=(0,0,p))
    out = F.conv3d(out, win.transpose(3, 4), groups=input.size(1), padding=(0,p,0))
    out = F.conv3d(out, win.transpose(2, 4), groups=input.size(1), padding=(p,0,0))
    return out

def filter_dilation(input, thresh=0.2, size=5):
    return filter_box_separated((input > thresh).float(), size) > 0
...
def crop_pad(sim, thresh=0.1, pad=0):
    ...
    if nz.numel() == 0:
        return others, ((0,0,0), (None,None,None))
...
def apply_bilateral_solver3d(t: torch.Tensor, r: torch.Tensor, c: torch.Tensor = None, grid_params={}, bs_params={}):
    ...
    if c is None:
        c = np.ones(shap).reshape(-1,1) * 0.9
    else:
